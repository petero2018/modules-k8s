...
module "spot_provisioner" {
  for_each = var.provisioner_enable_spot ? var.provisioners : {}

  source = "../karpenter-provisioner"

  provisioner_name = "${each.key}-spot"
  ...
  taints      = concat(coalesce(each.value.taints, []), local.spot_taint)
