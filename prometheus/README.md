...
| <a name="input_retention"></a> [retention](#input\_retention) | Prometheus data retention period. | `string` | `"15d"` | no |
| <a name="input_scrape_interval"></a> [scrape\_interval](#input\_scrape\_interval) | How frequently to scrape targets. | `string` | `"15s"` | no |
| <a name="input_timeout"></a> [timeout](#input\_timeout) | Time in seconds to wait for release installation. | `number` | `1200` | no |
| <a name="input_volume_size"></a> [volume\_size](#input\_volume\_size) | Prometheus server data Persistent Volume size. | `string` | `"50Gi"` | no |
